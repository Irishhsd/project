...
BEGIN
	...
	(
		...
		[Configuration_Name]	VARCHAR(255) NOT NULL, --Nombre de la configuración
		[Value]					VARCHAR(255) NOT NULL, --Valor
		[Created_At]			DATETIME NOT NULL DEFAULT GETDATE(), --Fecha de creación de la configuración
		[Updated_At]			DATETIME NOT NULL DEFAULT GETDATE(), --ültima fecha de cambio de configuración
...
BEGIN
  EXEC sys.sp_updateextendedproperty 'MS_Description', 'Define las configuraciones específicas para los productos, como diferentes colores y tamaños', 
  'SCHEMA', 'dbo', 'table', 'Product_Configurations'
END 
ELSE
BEGIN
  EXEC sys.sp_addextendedproperty 'MS_Description', 'Define las configuraciones específicas para los productos, como diferentes colores y tamaños',
  'SCHEMA', 'dbo', 'table', 'Product_Configurations'
END
GO

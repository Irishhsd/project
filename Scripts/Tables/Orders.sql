...
BEGIN
	...
	(
		[Id] INT NOT NULL PRIMARY KEY IDENTITY(1,1), --Identificador únicol
		[Total price] DECIMAL (20, 2) NOT NULL, --Precio total del pedido
		[Status] VARCHAR(50) NOT NULL, --Estado del pedido (e.g., pendiente, e)
		[Created_At] DATETIME NOT NULL DEFAULT GETDATE(), --Fecha de creación de la orden
		[Update_At] DATETIME NOT NULL DEFAULT GETDATE(), --Fecha de última actualización de la orden
	)
END
...
IF NOT EXISTS(SELECT * FROM ::fn_listextendedproperty ('MS_Description', 'SCHEMA', 'dbo', 'table', 'Orders', 'column', 'Created_At'))
    EXEC sp_addextendedproperty 'MS_Description', 'Fecha de creación de la orden', 'Schema', [dbo], 'table', [Orders], 'column', [Created_At];

IF NOT EXISTS(SELECT * FROM ::fn_listextendedproperty ('MS_Description', 'SCHEMA', 'dbo', 'table', 'Orders', 'column', 'Update_At'))
    EXEC sp_addextendedproperty 'MS_Description', 'Fecha de última actualización de la orden', 'Schema', [dbo], 'table', [Orders], 'column', [Update_At];
